...
void main()
{
    ...
    // ==================== Specular Term (Ashikkmin-Shirley) ====================
        
    // Calculate anisotropic exponent p
    // p = (n_u * (H·T)^2 + n_v * (H·B)^2) / (1 - (H·N)^2)
    float p = (n_u * HdotT * HdotT + n_v * HdotB * HdotB) / max(1.0 - (NdotH * NdotH), 0.0001);

    // Full Ashikhmin-Shirley specular formula from SIGGRAPH 2000 paper:
    // ρ_s = √((n_u+1)(n_v+1)) / (8π) * (n·h)^p / (k·h * max(n·k1, n·k2)) * F(k·h)
